...
def _release_result(session, operation_id, release_kind, *, lease_epoch=0):
    result = _service_result(session, operation_id, ServiceAction.REMOVE, lease_epoch=lease_epoch)
...
def test_full_happy_path_requires_release_and_service_evidence():
    ...
    donor_release = _release_result("donor", "op-donate", "DONOR_SLEEP_RELEASED")
    sm.advance("l1", LeaseState.DONOR_RELEASED, supporting_result=donor_release)
    assert sm.get("l1").last_release_digest == donor_release.release.header.digest

    sm.advance("l1", LeaseState.BORROWER_PREPARING)
    sm.advance("l1", LeaseState.BORROWER_ACTIVE, supporting_result=_service_result("borrower", "op-add", ServiceAction.ADD))
    sm.advance("l1", LeaseState.RECALLING)
    borrower_release = _release_result("borrower", "op-remove", "BORROWER_RUNTIME_DESTROYED")
    sm.advance("l1", LeaseState.BORROWER_RELEASED, supporting_result=borrower_release)
    sm.advance("l1", LeaseState.DONOR_RESTORING)
    sm.advance("l1", LeaseState.CLOSED, supporting_result=_service_result("donor", "op-restore", ServiceAction.ADD))
...
def test_release_must_reference_matching_service_removal_proof():
    sm = _machine()
    sm.advance("l1", LeaseState.DONOR_DRAINING)
    result = _release_result("donor", "op-donate", "DONOR_SLEEP_RELEASED")
    bad_release = ReleaseEvidence(
        ...
        released_gpu_uuids=result.release.released_gpu_uuids,
    )
    bad = OperationResult(
        ctx=result.ctx,
        target=result.target,
    ...
    with pytest.raises(MissingEvidenceError, match="service-removal"):
        sm.advance("l1", LeaseState.DONOR_RELEASED, supporting_result=bad)


def test_release_kind_and_participant_must_match_lease_edge():
    sm = _machine()
    sm.advance("l1", LeaseState.DONOR_DRAINING)
    wrong_kind = _release_result("donor", "op-donate", "BORROWER_RUNTIME_DESTROYED")
    ...
    wrong_session = _release_result("borrower", "op-donate-2", "DONOR_SLEEP_RELEASED")
...
def test_stale_lease_epoch_cannot_advance_release_edge():
    sm = _machine()
    sm.advance("l1", LeaseState.DONOR_DRAINING)
    stale = _release_result("donor", "op-donate", "DONOR_SLEEP_RELEASED", lease_epoch=1)
...
def test_service_activation_requires_add_evidence_not_status_only():
    sm = _machine(state=LeaseState.BORROWER_PREPARING, release_digest="release-d1")
    ...
    with pytest.raises(MissingEvidenceError, match=r"ServiceEvidence\(ADD\)"):
        sm.advance("l1", LeaseState.BORROWER_ACTIVE, supporting_result=remove_result)
