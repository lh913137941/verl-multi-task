...
- [x] OperationJournal 的 operation replay 校验覆盖完整业务身份、lease、command_seq、target 与摘要；`remaining_budget_ms` 不参与业务摘要，同号重试不能新建另一场操作或重置原受理操作的总时限。
- [x] OperationJournal 统一承载“每任务一场生命周期操作”和严格递增 command_seq fence；TaskRunner 只负责合同入口与结果投影，不再维护第二份 active-op / command-seq 可变状态。
...
- [x] 删除未接入生产 Trainer 的 `ScaleTransaction` 平行编排层及其 Protocol/Fake 测试；ADD/REMOVE 顺序只保留真实 owner 接口与证据约束这一套表达。
- [x] 空泡候选判定内聚到 `select_idle_candidates`，删除只为该 selector 服务的 `candidate()` 辅助 API 与对应重复测试。
...
- 纯 Python 契约、journal、exactly-once、ledger、lease、owner commit evidence 与 idle-candidate selection 为本轮优先回归层。
